...
orderRouter.put("/api/orders", async (req, res) => {
    try {
        const {orderID, driverID, carID, date, time, state} = req.body;
        await OrderService.UpdateOrder(orderID, driverID, carID, date, time, state); // вызываем функцию обновления заказа
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
